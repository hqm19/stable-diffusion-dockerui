...
# 在 start() 函数开始时，设置一个退出点，以使 docker build 过程不要直接启动 webui 服务
RUN sed -i '/def start():/a \ \ \ \ \n\ \ \ \ # ------ make a docker build cache point ------ \n\ \ \ \ if os.getenv("EXIT_ON_IMAGE_BUILD") == "1":\n\ \ \ \ \ \ \ \ print("EXIT_ON_IMAGE_BUILD is 1, exiting.")\n\ \ \ \ \ \ \ \ sys.exit()\n' ./modules/launch_utils.py && \
    # 将 clip-vit-large-patch14 的路径改为本地路径，避免从 huggingface.co 下载
...
# ----- 用法 -----
# 构建镜像：
# nohup docker build --build-arg PROXY=10.10.0.8:10887 --build-arg CACHEBUST=$(date +%s) -t stable-diffusion-webui:20231206 . > build.log 2>&1 &
# 用构建好的镜像，启动交互式容器：
# docker run --gpus all -it -p 7860:7860 -v "${PWD}/models:/home/sd-webui/models" -v "${PWD}/extensions:/home/sd-webui/extensions" -v"${PWD}/huggingface.co:/home/sd-webui/huggingface.co" stable-diffusion-webui:20231207 bash
# 用构建好的镜像，启动后台运行容器：
# docker run --gpus all -d -p 7880:7880 -v "${PWD}/models:/home/sd-webui/models" -v "${PWD}/extensions:/home/sd-webui/extensions" -v"${PWD}/huggingface.co:/home/sd-webui/huggingface.co" stable-diffusion-webui:20231207
#